...
const SensorSpaceCoordinateEditor: React.FunctionComponent<IWidgetProps> = (
  props
) => {
  ...
  const saveConfigToBackend = React.useCallback(
    async (configToSave: IConfig) => {
      try {
  ...
  const saveRegionChanges = React.useCallback(async () => {
    ...
    try {
      ...
      await props.uxpContext?.executeAction(model, action, params, {
        json: true,
      });
      await loadSpaces();
  ...
  const handleRegionUpdate = React.useCallback(
    (index: number, newPosition: IRegion) => {
      setRegion((prev) =>
  ...
  const markers = React.useMemo((): IMarker[] => {
    ...
    const createCustomHTMLMarker = (
      ...
      const defaultMarker = `<div class='space-editor__default-marker ${
        isSelected ? "space-editor__default-marker--selected" : ""
      }' style="border-color: ${color}; background-color: ${color}33;">
                                <div div class="space-editor__default-marker-icon" style="background-color: ${color};"></div>
                            </div>`;
      const customMarker = `<div class="space-editor__icon-marker ${
        isSelected ? "space-editor__icon-marker--selected" : ""
    ...
    const createMarkers = (
      ...
      return region.map((pos, index) => {
        ...
        const m: any = {
          ...
          customHTMLIcon: createCustomHTMLMarker(
            true,
            isEditing || false,
            color,
    ...
    if (selectedSpace) {
      const selectedKind = filteredSpaceRegions.find(
        (s) => s.spaceId === selectedSpace.id
      )?.type;
      const isMarker = selectedKind === "marker" || selectedKind === "unplaced";
      if (isMarker) {
        allMarkers.push(
          ...createMarkers("marker", selectedSpace?.icon, isEditingRegion)
        );
  ...
  const mapRegions = React.useMemo(() => {
    ...
    filteredSpaceRegions
      .filter(
        (item) => item.type === "region" && selectedSpace?.id !== item.spaceId
      )
      .forEach((spaceRegion) => {
  ...
  const updateSelectedSpace = React.useCallback(
    (space?: ISpace) => {
      if (!space) return;
      ...
      <div className="space-editor">
        <div className="space-editor__header">
          ...
          <div className="space-editor__actions">
            <div className="space-editor__settings">
              <div
                className="space-editor__settings-button"
        ...
        <div className="space-editor__configuration-needed">
          ...
          <div className="space-editor__configuration-needed-message">
            Please configure the data models and actions to get started with the
            coordinate editor.
          </div>
          <Button
            icon="fas tools"
  ...
  return (
    <div className="space-editor">
      ...
      <div
        className={`space-editor__body ${
          selectedSpace ? "space-editor__body--editing" : ""
        }`}
      >
        <div className="space-editor__sidebar">
          ...
          <div className="space-editor__list">
            {filteredSpaceRegions.map((space, index) => (
              <div
                key={space.space.id}
                className={`space-editor__space ${
                  selectedSpace?.id === space.space.id
                    ? "space-editor__space--active"
                    : ""
                }`}
                onClick={() => handleSpaceSelect(space.space)}
              >
                {config?.showIdInsteadOfName
                  ? space.space.id
                  : space.space.name}
              </div>
            ))}
            {filteredSpaceRegions?.length == 0 && (
              <div className="space-editor__list--empty">No spaces found</div>
            )}
          </div>
        </div>
        ...
        <div className="space-editor__map-container">
          {selectedFloorData?.layout?.floorPlan ? (
            ...
            <div className="space-editor__no-map">
              Select a floor to get started
            </div>
          )}
        </div>

        {selectedFloorData?.layout?.floorPlan && selectedSpace && (
          <div className="space-editor__map-toolbar">
            <div className="space-editor__map-toolbar-header">
              <div className="space-editor__map-toolbar-title">Coordinates</div>
              <div
                className="space-editor__map-toolbar-button"
                onClick={() => {
                  setIsConfirming(false);
                  setIsSaving(false);
                  setIsEditingRegion(false);
                    ...
                    <tr key={index} className="space-editor__coordinate-row">
                      ...
                      <td>
                        <Input
                          ...
                          onChange={(value) =>
                            handleCoordinateChange(index, "x", value)
                          }
                          placeholder="X"
                      ...
                      <td>
                        <Input
                          ...
                          onChange={(value) =>
                            handleCoordinateChange(index, "y", value)
                          }
                          placeholder="Y"
                          className="space-editor__coordinate-input"
                          inputAttr={{
